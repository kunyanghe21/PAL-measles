...
CACHE_DIR = "wwr/E12"
CACHE_FILE = os.path.join(CACHE_DIR, "E12_param_exp.npz")
CACHE_KEY = "E12_param_exp"
os.makedirs(CACHE_DIR, exist_ok=True)

if os.path.exists(CACHE_FILE):
    print(f"[cache] Found existing results → {CACHE_FILE}")
    E12_param_exp = np.load(CACHE_FILE)[CACHE_KEY]
    print("Optimization done! final x =", E12_param_exp)
else:
    plt.ioff()
    ...
    x_0 = np.array([
    -2.346007,   # x[0] = log(S_0)              ;  S_0  = 0.09586288
 -7.457166,   # x[1] = log(E_0)              ;  E_0  = 0.0005771460
 -7.294279,   # x[2] = log(I_0)              ;  I_0  = 0.0006808579
  0.124676,   # x[3] = log(beta_bar)         ;  beta_bar = 1.1328356
 -0.934994,   # x[4] = log(rho)              ;  rho  = 0.19638592
 -2.525789,   # x[5] = log(gamma)            ;  gamma = 0.08026491
  2.021956,   # x[6] = log(g / 100)          ;  g/100 = 7.5612555
 -6.398584,   # x[7] = log(a)                ;  a    = 0.0016601314
 -0.556904,   # x[8] = log(c)                ;  c    = 0.2862406
 -3.158640,   # x[9] = log(sigma_xi / 10)    ;  sigma_xi / 10 = 0.042527638
 -4.557204    # x[10]= log(q_var)            ;  q_var = 0.010486702
]          
    )
    ...
    E12_param = res.x
    print("Optimization done! final x =", E12_param)

    E12_param_exp = np.exp(E12_param)
    np.savez(CACHE_FILE, **{CACHE_KEY: E12_param_exp})
