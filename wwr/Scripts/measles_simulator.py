...
def compute_infection_rate(X_tm1_bar, is_school_term_array_t, pop_t, p, a, beta_bar, v, xi_t):
    ...
    spatial_infection = infected_prop_t + tf.reduce_sum((v / pop_t) * (tf.transpose(infected_prop_t) - infected_prop_t),
...
@tf.function(jit_compile=True)
def scan_intermediate(X_tprev, K_tprev, is_start_school_year_array_t_obs, intermediate_steps, UKbirths_t, c, n_cities,
                      delta_year):
    def body(input, t_intermediate):
        ...
        X_birth_tm1 = tf.concat(
            (birth_tm1, tf.zeros(tf.shape(birth_tm1)), tf.zeros(tf.shape(birth_tm1)), tf.zeros(tf.shape(birth_tm1))),
            axis=-1)
...
@tf.function(jit_compile=True)
def body_run(X_tm1, intermediate_steps, UKbirths_t, pop_t, beta_bar, p, a, is_school_term_array_t,
             is_start_school_year_array_t_obs, h, rho, gamma, xi_t, q_t, c, n_cities, delta_year, v):
    ...
    X_t, Z_t = scan_intermediate(X_tm1, K_t, is_start_school_year_array_t_obs, intermediate_steps, UKbirths_t, c,
                                 n_cities, delta_year)
...
def run(T, intermediate_steps, UKbirths, UKpop, g, measles_distance_matrix, initial_pop, pi_0, beta_bar, p, a,
        is_school_term_array, is_start_school_year_array, h, rho, gamma, Xi, Q, c, n_cities, delta_year):
    ...
    def body(input, t_obs):
        ...
        output = body_run(X_t, intermediate_steps, UKbirths_t, pop_t, beta_bar, p, a, is_school_term_array_t,
                          is_start_school_year_array_t_obs, h, rho, gamma, xi_t, q_t, c, n_cities, delta_year, v)
