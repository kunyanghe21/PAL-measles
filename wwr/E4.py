...
from Scripts.corrected_euler_time_step import PAL_run_likelihood_res
...
pi_0_1 = 0.02536        # S₀  = e^(-3.674239)
pi_0_2 = 0.0042         # E₀  = e^(-5.472271)
pi_0_3 = 0.000061    # I₀  = e^(-9.705021)
pi_0   = (
...
beta_bar = tf.convert_to_tensor(6.30 * tf.ones((n_cities, 1)), dtype=tf.float32)   # e^(1.840549)
rho      = tf.convert_to_tensor([0.142], dtype=tf.float32) * tf.ones((n_cities, 1), dtype=tf.float32)  # e^(-1.949856)
gamma    = tf.convert_to_tensor([0.0473], dtype=tf.float32) * tf.ones((n_cities, 1), dtype=tf.float32)  # e^(-3.051457)

# g — not in the current x vector; keep fixed (here 0)
g = tf.convert_to_tensor([[0.0]], dtype=tf.float32) * tf.ones((n_cities, 1), dtype=tf.float32)

# a , c , σξ , qvar
a      = tf.constant(0.1476, dtype=tf.float32)                    # e^(-1.918923)
c      = tf.constant(0.219 , dtype=tf.float32)                    # e^(-1.517331)
xi_var = tf.convert_to_tensor(0.318 , dtype=tf.float32)            # 10·e^(-3.446486)
q_var  = tf.convert_to_tensor(0.305, dtype=tf.float32)            # e^(-1.186399)
