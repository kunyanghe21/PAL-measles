...
CACHE_DIR = "wwr/E4"
CACHE_FILE = os.path.join(CACHE_DIR, "E4_param_exp.npz")
CACHE_KEY = "E4_param_exp"

os.makedirs(CACHE_DIR, exist_ok=True)

if os.path.exists(CACHE_FILE):
    print(f"[cache] Found existing results → {CACHE_FILE}")
    E4_param_exp = np.load(CACHE_FILE)[CACHE_KEY]
else:
 ...
 -1.184170    
    ...
    def optimization_func(x_optim):
        ...
        value = -PAL_run_likelihood_lookahead(
            T,
            intermediate_steps,
            UKmeasles,
            UKbirths,
    ...
    E4_param = res.x
    print("Optimization done! final x =", E4_param)

    E4_param_exp = np.exp(E4_param)
    np.savez(CACHE_FILE, **{CACHE_KEY: E4_param_exp})
