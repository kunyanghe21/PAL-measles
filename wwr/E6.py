...
# ----------------------------------------------------------------
    ...
    h = tf.constant(
        14 / tf.cast(intermediate_steps, dtype=tf.float32),
        dtype=tf.float32
    )
    (
        ...
        times_total,
        times_obs,
    ) = school_term_and_school_year(
        T, intermediate_steps, term, school
    )

    is_school_term_array = tf.convert_to_tensor(is_school_term_array,
                                                dtype=tf.float32)
    is_start_school_year_array = tf.convert_to_tensor(
        is_start_school_year_array, dtype=tf.float32
    )
    p = tf.constant(0.759, dtype=tf.float32)
    delta_year = (
        tf.convert_to_tensor([[1 / 50]], dtype=tf.float32)
        * tf.ones((n_cities, 4), dtype=tf.float32)
    )

    # increase the n_experiments for proper variance estimates,
    # use n_experiments to just test the log-likelihood
    n_experiments = 20

    # lookahead
    # shared
    best_parameters = np.load(
        "wwr/Data/Parameter/final_parameters_lookahead_A.npy"
    )
    ...
    # --- parameter block (values from latest optimisation) ---------------

    # π₀ —— initial state fractions
    pi_0_1 = 0.0217611245659969   # S₀ = e^{-5.30681518}
    pi_0_2 = 0.00289588429752681   # E₀ = e^{-4.97504388}
    pi_0_3 = 6.00344202801694e-05    # I₀ = e^{-8.69451886}
    pi_0 = (
    ...
    beta_bar = tf.convert_to_tensor(
        3.32510580226645* tf.ones((n_cities, 1)), dtype=tf.float32
    )   # e^{1.06649407}
    rho = tf.convert_to_tensor(
        [[0.0433265236952158]], dtype=tf.float32
    ) * tf.ones((n_cities, 1), dtype=tf.float32)  # e^{-2.15487041}
    gamma = tf.convert_to_tensor(
        [[0.0604352096792782]], dtype=tf.float32
    ) * tf.ones((n_cities, 1), dtype=tf.float32)  # e^{-3.14631130}

    # g  (fixed externally, here 0)
    g = tf.convert_to_tensor(
        [[0.0]], dtype=tf.float32
    ) * tf.ones((n_cities, 1), dtype=tf.float32)

    # a , c , σξ , qvar
    a = tf.constant(0.610617219783711, dtype=tf.float32)        # e^{-1.76877006}
    c = tf.constant(0.892217738319218, dtype=tf.float32)        # e^{-1.36659790}
    xi_var = tf.convert_to_tensor(
        0.526681063214281, dtype=tf.float32
    )   # 10·e^{-2.80183737}
    q_var = tf.convert_to_tensor(
        0.052499902404982, dtype=tf.float32
    )   # e^{-2.06775914}

    # prior distributions
    Xi = tfp.distributions.Gamma(concentration=xi_var, rate=xi_var)
    Q = tfp.distributions.TruncatedNormal(
        loc=0.50, scale=q_var, low=0.0, high=1.0
    )
    ...
    def logmeanexp(x, se=False, ess=False):
        ...
        if se:
            ...
            for k in range(n):
                ...
                jk_vals[k] = max_x_mk + np.log(
                    np.mean(np.exp(x_minus_k - max_x_mk))
                )
    ...
    out_file_path = os.path.join("E6", "PAL_real.csv")
