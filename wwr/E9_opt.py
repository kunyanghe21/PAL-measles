...
CACHE_DIR = "wwr/E9"
CACHE_FILE = os.path.join(CACHE_DIR, "E9_param_exp.npz")
CACHE_KEY = "E9_param_exp"
os.makedirs(CACHE_DIR, exist_ok=True)

if os.path.exists(CACHE_FILE):
    print(f"[cache] Found existing results → {CACHE_FILE}")
    E9_param_exp = np.load(CACHE_FILE)[CACHE_KEY]
    print("Optimization done! final x =", E9_param_exp)
else:
    plt.ioff()
    ...
    E9_param = res.x
    print("Optimization done! final x =", E9_param)

    E9_param_exp = np.exp(E9_param)
    np.savez(CACHE_FILE, **{CACHE_KEY: E9_param_exp})
