...
# -----------------------------------------------------------------
    ...
    # β̄ , ρ , γ
    beta_bar = tf.convert_to_tensor(1.144799 * tf.ones((n_cities, 1)), dtype = tf.float32)
    rho      = tf.convert_to_tensor([[0.393589]], dtype = tf.float32) * tf.ones((n_cities, 1), dtype = tf.float32)
    gamma    = tf.convert_to_tensor([[0.079675]], dtype = tf.float32) * tf.ones((n_cities, 1), dtype = tf.float32)

    # g  = 100 · e^{2.030066}
    g = tf.convert_to_tensor([[761.459]], dtype = tf.float32) * tf.ones((n_cities, 1), dtype = tf.float32)

    # a , c , σξ , qvar
    a      = tf.constant(0.001691, dtype = tf.float32)
    c      = tf.constant(0.543432, dtype = tf.float32)
    xi_var = tf.convert_to_tensor(0.416993, dtype = tf.float32)
    q_var  = tf.convert_to_tensor(0.010619, dtype = tf.float32)
    ...
    np.savetxt(
        os.path.join("wwr", "E12", "PAL_lookahead_40.csv"),
        log_likelihood_shared,
        delimiter = ",",
    )
