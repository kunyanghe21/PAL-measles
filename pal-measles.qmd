...
\bottomrule
\end{tabular}
\caption{Log‐likelihood estimates ($\lambda_k$) and their standard errors ($\sigma_k$). ARMA gives the log-likelihood for an autoregressive moving average benchmark, and NegBinom is an autoregressive negative binomial benchmark.}
...
The smaller number of particles leads for $E_4$ leads to a higher log-likelihood estimate and a lower Monte Carlo error estimate, both of which are suggestive of a long left tail to the Monte Carlo likelihood estimate leading to estimators that behave as though the variance is infinite.
