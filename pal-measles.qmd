...
\bottomrule
\end{tabular}
\caption{Log‐likelihood estimate, $\lambda$, and its standard error, $\sigma$, for each experiment described in Table\ \ref{tbl:treatments}. ARMA gives the log-likelihood for an autoregressive moving average benchmark, and NegBinom is an autoregressive negative binomial benchmark.}
