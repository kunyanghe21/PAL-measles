...
E1_result_vec <- c(as.numeric(E1_result[1]),  as.numeric(E1_result[2]))
E8_result_vec <- c(as.numeric(E8_result[1]),  as.numeric(E8_result[2]))
...
\toprule
\textbf{$E$} & $E_1$ & $E_2$ & $E_3$ & $E_4$ & $E_5$ & $E_8$ & $E_9$ & $E_{10}$ \\
\midrule
$\lambda_k$ &
...
\bottomrule
\end{tabular}
\caption{Log‐likelihood estimates ($\lambda_k$) and their standard errors ($\sigma_k$)}
