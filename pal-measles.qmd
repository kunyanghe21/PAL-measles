...
date: "May 28, 2025"
abstract: |
  Filtering algorithms for high-dimensional nonlinear non-Gaussian partially observed stochastic processes provide access to the likelihood function and hence enable likelihood-based or Bayesian inference.
...
The experimental variables, and the list of values we consider for them, are summarized in Table\ \ref{tbl:description} and further described below.
...
For $E_9$, $E_{10}$, $E_{12}$--$E_{15}$ we use the parameter values published by WWR for their case A.
...
This suggests a long left tail to the Monte Carlo likelihood estimate, leading to estimators that behave as though the variance is infinite.
...
We see that the $U=40$ results scale approximately linearly compared to $U=1$, providing us with supporting evidence for our hypothesis Q2.
...
Experiments $E_{14}$ and $E_{15}$ directly use the parameters provided by WWR for their model A.
Experiments $E_{12}$ and $E_{13}$ optimize $\PALV$ using the code from WWR, since results for $\PALV$ were not included in their paper.
The choice of Model A in $E_{12}$--$E_{15}$ was motivated by computational convenience and to rapidly demonstrate the persistence of positive bias in the lookahead method for the 40-unit dataset.
In particular, the comparison of $E_{14}$ and $E_{15}$ shows very directly the bias of $\PALL$, using the code and parameter values of WWR and changing only the number of particles.
The decrease of the log-likelihood by  $\lambda_{14}-\lambda_{15}=$ `r sprintf('%.2f', E15_lambda-E14_lambda)` log units when moving from $J_1=5\times 10^3$ to $J_2=10^5$ is strong evidence that the value with $J=5\times 10^3$ is over-optimistic.
